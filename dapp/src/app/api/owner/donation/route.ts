...
import { getAllCampaignsFromDbForOwnerByWalletAddress, getAllOwnerDonationFromDbByWalletAddress } from "@/services/controller/campaign";
...
async function getHandler(req: NextRequest) {
    ...
    try {
        ...
        if (data && owner && owner.email === session.user.email) {
            // getting all the donation that owner did
            // Logic here
            const res = await getAllOwnerDonationFromDbByWalletAddress(data)
            if (res) {
